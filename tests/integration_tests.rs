use life_of_pi::{SystemCollector, SystemMonitor, metrics::data::SystemSnapshot};
...
async fn test_system_collector_basic_functionality() {
    let mut collector = SystemCollector::new()
        .expect("Should create SystemCollector");
...
async fn test_snapshot_serialization_roundtrip() {
    ...
    let original_snapshot = collector.get_snapshot().await
        .expect("Should collect system snapshot");
    
    // Serialize to JSON
    let json_string = serde_json::to_string(&original_snapshot)
        .expect("Should serialize snapshot to JSON");
    
    // Deserialize back
    let deserialized_snapshot: SystemSnapshot = serde_json::from_str(&json_string)
        .expect("Should deserialize snapshot from JSON");
    
    // Verify key fields match
    assert_eq!(original_snapshot.cpu.cores, deserialized_snapshot.cpu.cores);
    assert_eq!(original_snapshot.memory.total_bytes, deserialized_snapshot.memory.total_bytes);
    assert_eq!(original_snapshot.system.hostname, deserialized_snapshot.system.hostname);
    assert_eq!(original_snapshot.system.os_name, deserialized_snapshot.system.os_name);
}
...
async fn test_memory_stability_over_time() {
    ...
    let final_snapshot = collector.get_snapshot().await
    ...
    if initial_mem > 0 && final_mem > 0 {
        ...
        assert!(diff_ratio < 0.5, "Memory usage should remain relatively stable. Initial: {}, Final: {}, Ratio: {}", initial_mem, final_mem, diff_ratio);
    } else {
        ...
        assert_eq!(initial_mem, final_mem, "Memory values should at least be consistent");
    }
}
...
async fn test_temperature_data_consistency() {
    ...
    if let Some(cpu_temp) = snapshot.temperature.cpu_celsius {
        assert!(cpu_temp > -50.0 && cpu_temp < 150.0, "CPU temperature should be reasonable: {}", cpu_temp);
    }
    
    if let Some(gpu_temp) = snapshot.temperature.gpu_celsius {
        assert!(gpu_temp > -50.0 && gpu_temp < 150.0, "GPU temperature should be reasonable: {}", gpu_temp);
    }
    
    // Thermal zones should have valid names if present
    for (zone_name, temp) in &snapshot.temperature.thermal_zones {
        assert!(!zone_name.is_empty(), "Thermal zone name should not be empty");
        assert!(*temp > -100.0 && *temp < 200.0, "Thermal zone temperature should be reasonable: {}", temp);
    }
}
...
async fn test_network_interface_data() {
    ...
    assert!(!snapshot.network.is_empty(), "Should have at least one network interface");
...
async fn test_disk_usage_data() {
    ...
    assert!(!snapshot.storage.is_empty(), "Should have at least one disk");
    
    for disk in &snapshot.storage {
        assert!(!disk.device.is_empty(), "Disk device name should not be empty");
        ...
        assert!(disk.available_bytes <= disk.total_bytes, "Available space should not exceed total");
    }
}
...
async fn test_snapshot_timestamp_accuracy() {
    ...
    assert!(snapshot.timestamp >= before, "Snapshot timestamp should not be in the past");
    assert!(snapshot.timestamp <= after, "Snapshot timestamp should not be in the future");
}
...
async fn test_cpu_load_average_validity() {
    ...
    assert!(snapshot.cpu.load_average.one_minute >= 0.0, "1-minute load average should be non-negative");
    assert!(snapshot.cpu.load_average.five_minutes >= 0.0, "5-minute load average should be non-negative");
    assert!(snapshot.cpu.load_average.fifteen_minutes >= 0.0, "15-minute load average should be non-negative");
    
    // Per-core usage should be valid percentages
    for (core, usage) in snapshot.cpu.core_usage.iter().enumerate() {
        assert!(*usage >= 0.0 && *usage <= 100.0, "Core {} usage should be between 0-100%: {}", core, usage);
    }
    
    // Overall CPU usage should be a valid percentage
    assert!(snapshot.cpu.usage_percent >= 0.0 && snapshot.cpu.usage_percent <= 100.0, 
            "Overall CPU usage should be between 0-100%: {}", snapshot.cpu.usage_percent);
}
...
async fn test_snapshot_performance() {
    ...
    assert!(duration < Duration::from_secs(1), "Snapshot collection took too long: {:?}", duration);
...
async fn test_json_serialization_performance() {
    ...
    assert!(duration < Duration::from_millis(100), "JSON serialization took too long: {:?}", duration);
...
async fn test_data_consistency_across_collections() {
    ...
    let snapshot1 = collector.get_snapshot().await
    ...
    assert_eq!(snapshot1.cpu.cores, snapshot2.cpu.cores, "CPU core count should be consistent");
    assert_eq!(snapshot1.cpu.model, snapshot2.cpu.model, "CPU model should be consistent");
    assert_eq!(snapshot1.system.hostname, snapshot2.system.hostname, "Hostname should be consistent");
    
    // Timestamps should be different and in order
    assert!(snapshot2.timestamp >= snapshot1.timestamp, "Timestamps should be in order");
}
...
async fn test_error_handling_graceful_degradation() {
    ...
    assert!(collector_result.is_ok(), "Collector creation should not fail on supported systems");
    ...
    if let Ok(mut collector) = collector_result {
        ...
        assert!(snapshot_result.is_ok(), "Should handle partial metric collection gracefully");
    }
}