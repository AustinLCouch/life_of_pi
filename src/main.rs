...
use life_of_pi::{
    SystemCollector, SystemMonitor, WebConfig,
    start_web_server, DEFAULT_INTERVAL_MS, DEFAULT_WEB_PORT,
};
...
async fn serve_command(cli: &Cli, args: &ServeArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stream = collector.start_collecting_with_interval(cli.interval).await?;
    info!("Started metrics collection with {}ms interval", cli.interval);
...
async fn info_command() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  OS: {} {}", snapshot.system.os_name, snapshot.system.os_version);
    println!("  Kernel: {}", snapshot.system.kernel_version);
    println!("  Uptime: {} seconds", snapshot.system.uptime_seconds);
    println!();
    
    println!("Hardware:");
    println!("  CPU: {} ({} cores)", snapshot.cpu.model, snapshot.cpu.cores);
    println!("  Architecture: {}", snapshot.cpu.architecture);
    println!("  Memory: {:.1} GB total", snapshot.memory.total_bytes as f64 / 1024.0 / 1024.0 / 1024.0);
    ...
    for iface in &snapshot.network {
        println!("  {}: {}", iface.interface, if iface.is_up { "UP" } else { "DOWN" });
    }
...
fn print_pretty_snapshot(snapshot: &life_of_pi::SystemSnapshot) {
    ...
    // Memory info
    println!("🧠 Memory:");
    println!("  Total: {:.1} GB", snapshot.memory.total_bytes as f64 / 1024.0 / 1024.0 / 1024.0);
    println!("  Available: {:.1} GB", snapshot.memory.available_bytes as f64 / 1024.0 / 1024.0 / 1024.0);
    println!("  Usage: {:.1}%", snapshot.memory.usage_percent);
    println!("  Swap: {:.1} GB used", snapshot.memory.swap.used_bytes as f64 / 1024.0 / 1024.0 / 1024.0);
    println!();
