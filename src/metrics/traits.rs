...
pub trait MetricsProvider {
    ...
    fn collect_snapshot(&mut self) -> impl std::future::Future<Output = Result<SystemSnapshot>> + Send;
    
    /// Start continuous collection of system metrics.
    /// 
    /// Returns a stream of system snapshots that are collected at the
    /// specified interval.
    fn start_stream(&mut self, interval_ms: u64) -> impl std::future::Future<Output = Result<BoxStream<'static, SystemSnapshot>>> + Send;
}
...
pub trait SystemMonitor {
    ...
    fn start_collecting(&mut self) -> impl std::future::Future<Output = Result<BoxStream<'static, SystemSnapshot>>> + Send;
    
    /// Start collecting system metrics with custom interval.
    fn start_collecting_with_interval(&mut self, interval_ms: u64) -> impl std::future::Future<Output = Result<BoxStream<'static, SystemSnapshot>>> + Send;
